...
# Implementation of the WordsAPI interface created by Ashley Williams.
# Find the project at https://github.com/ehwhydubya/pywords
...
class WordsAPI(object):
    ...
    def __init__(self, api_key, pretty=None):
        ...
        self._auth_headers = {'x-rapidapi-host': "wordsapiv1.p.rapidapi.com",
                              'x-rapidapi-key': api_key} #auth against Mashape APIs
    ...
    def setPrettyPrint(self, toggle):
        if toggle == 'on':
            ...
            raise WordsAPI.pretty_print_error
    ...
    def _get(self, word, detail):
        ...
        if not isinstance(word, str):
            raise WordsAPI.entry_error

        if detail not in WordsAPI._details:
            raise WordsAPI.detail_error

        url = self._base_url + WordsAPI._details[detail].format(word)
